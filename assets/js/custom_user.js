...
$(document).ready(function(){
	...
	$('#view').on('click', '.btn-form-ubah', function(){ // Ketika tombol dengan class btn-form-ubah pada div view di klik
        ...
        var nik = tr.find('.nik-value')
		var nama = tr.find('.nama-value').val() // Ambil nis dari input type hidden
		var user = tr.find('.user-value').val() // Ambil nama dari input type hidden
		var password = tr.find('.pas-value').val() // Ambil jenis kelamin dari input type hidden
		var telp = tr.find('.telp-value').val() // Ambil telepon dari input type hidden
		// var level = tr.find('.level-value').val() // Ambil alamat dari input type hidden

        $('#nik').val(nik)
		$('#nama').val(nama) // Set value dari textbox nis yang ada di form
		$('#user').val(user) // Set value dari textbox nama yang ada di form
		$('#pas').val(password) // Set value dari textbox nama yang ada di form
		$('#telp').val(telp) // Set value dari textbox nama yang ada di form
		// $('#level').val(level) // Set value dari textbox nama yang ada di form
	})
	...
	$('#btn-simpan').click(function(){ // Ketika tombol simpan di klik
		...
		$.ajax({
			url: base_url + 'petugas/simpan', // URL tujuan
	...
	$('#btn-ubah').click(function(){ // Ketika tombol ubah di klik
		...
		$.ajax({
			url: base_url + 'homepage/ubah_user/' + id, // URL tujuan
			type: 'POST', // Tentukan type nya POST atau GET
			data: $("#form-modal form").serialize(), // Ambil semua data yang ada didalam tag form
			dataType: 'json',
			beforeSend: function(e) {
	...
	$('#btn-hapus').click(function(){ // Ketika tombol hapus di klik
		...
		$.ajax({
			url: base_url + 'homepage/hapus_user/' + id, // URL tujuan
